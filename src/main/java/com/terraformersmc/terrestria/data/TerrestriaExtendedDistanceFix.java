...
public class TerrestriaExtendedDistanceFix extends DataFix {
	public TerrestriaExtendedDistanceFix(Schema schema, boolean changesType) {
		super(schema, changesType);
	}

	@Override
	public TypeRewriteRule makeRule() {
		return this.fixTypeEverywhereTyped("TerrestriaExtendedDistanceFix",
			this.getInputSchema().getType(TypeReferences.BLOCK_STATE), blockStateTyped -> blockStateTyped
				.update(DSL.remainderFinder(), TerrestriaExtendedDistanceFix::updateExtendedLeavesDistance));
	}

	private static <T> Dynamic<T> updateExtendedLeavesDistance(Dynamic<T> blockStateDynamic) {
		Collection<String> extendedLeavesIds = Set.of(
